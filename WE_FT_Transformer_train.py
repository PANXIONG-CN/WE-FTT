...
import sys
print("=" * 80, flush=True)
print("WE-FTT Training Started", flush=True)
print("=" * 80, flush=True)
...
if torch.cuda.is_available():
    ...
    print(f"Current GPU: {torch.cuda.get_device_name(0)}", flush=True)
else:
...
class Config:
    ...
    PATIENCE = 5
    NUM_TRIALS = 10
    FILE_PATH = "/mnt/hdd_4tb_data/ArchivedWorks/MBT/FTT/Mindformers/training_dataset_demo1.parquet"
    COLUMNS_FEATURES = [
        "BT_06_H",
        "BT_06_V",
        "BT_10_H",
...
class ParquetDataset(Dataset):
    def __init__(self, parquet_path: str, feature_cols: list, weight_cols: list, label_col: str, device: str = "cuda"):
        ...
        self.labels = torch.LongTensor(
            df[label_col].astype("int64").to_numpy())
...
def get_sinusoidal_encoding_table(seq_len, embed_dim):
    ...
    sinusoidal_table = np.array([get_posi_angle_vec(pos_i)
...
class CustomFTTransformerModel(nn.Module):
    def __init__(
        ...
        self.feature_proj = nn.Linear(
            len(columns_features), config.input_embed_dim)
        self.weight_proj = nn.Linear(
            len(columns_weights), config.input_embed_dim)
        self.layer_norm = nn.LayerNorm(
            config.input_embed_dim, eps=layer_norm_eps)

        self.cls_token = nn.Parameter(
            torch.zeros(1, 1, config.input_embed_dim))
        nn.init.normal_(self.cls_token, std=0.02)

        self.attn_blocks = nn.ModuleList(
            [MultiHeadedAttention(config.num_heads, config.input_embed_dim, head_dim,
                                  config.attn_dropout) for _ in range(config.num_attn_blocks)]
        )
        self.residual_layers = nn.ModuleList([nn.Linear(
            config.input_embed_dim, config.input_embed_dim) for _ in range(config.num_attn_blocks)])
        ...
        self.pos_enc = nn.Parameter(get_sinusoidal_encoding_table(
            seq_len=2, embed_dim=config.input_embed_dim), requires_grad=False)
    ...
    def update_alpha(self, epoch_stats):
        ...
        if len(self.epoch_stats) > 5:
            ...
            if self.current_alpha is None:
                ...
                self.current_alpha = self.momentum * \
                    self.current_alpha + (1 - self.momentum) * new_alpha

    def forward(self, inputs, targets):
        alpha = torch.FloatTensor(self.current_alpha).to(
            inputs.device) if self.current_alpha is not None else None
...
class WarmupCosineSchedule:
    ...
    def step(self):
        ...
        if self.current_step < self.warmup_steps:
            lr_scale = float(self.current_step) / \
            ...
            lr_scale = max(
                0.0, 0.5 * (1.0 + math.cos(math.pi * progress))) * self.lr_decay_factor
...
class Trainer:
    ...
    def objective(self, trial, train_dataset, test_dataset, class_weights):
        ...
        train_loader = DataLoader(train_dataset, batch_size=per_gpu_batch_size,
                                  shuffle=True, num_workers=self.config.NUM_WORKERS, pin_memory=False)
        test_loader = DataLoader(test_dataset, batch_size=per_gpu_batch_size,
                                 shuffle=False, num_workers=self.config.NUM_WORKERS, pin_memory=False)

        num_heads = trial.suggest_categorical("num_heads", [4, 8, 16, 32])
        input_embed_dim = trial.suggest_categorical(
            "input_embed_dim", [64, 128, 256, 512])
        ...
        fusion_hidden_dim = trial.suggest_categorical(
            "fusion_hidden_dim", [128, 256, 512, 1024])
        fusion_activation = trial.suggest_categorical(
            "fusion_activation", ["relu", "gelu", "silu"])
        ...
        layer_norm_eps = trial.suggest_float(
            "layer_norm_eps", 1e-7, 1e-5, log=True)
        ...
        criterion = DynamicFocalLoss(
            alpha=None, gamma=focal_gamma, reduction="mean", momentum=focal_momentum)
        ...
        scheduler = WarmupCosineSchedule(
            optimizer, warmup_steps=warmup_steps, total_steps=total_steps, lr_decay_factor=lr_decay_factor)
        ...
        for epoch in range(self.config.NUM_EPOCHS):
            ...
            for features_batch, weights_batch, y_batch in train_loader:
                ...
                with torch.amp.autocast(device_type="cuda" if torch.cuda.is_available() else "cpu"):
                    ...
                    advantages = self.compute_rewards_and_advantages(
                        predictions, y_batch)

                    log_probs = F.log_softmax(logits, dim=1)
                    selected_log_probs = log_probs.gather(
                        1, y_batch.unsqueeze(1)).squeeze(1)
                    pg_loss = -(selected_log_probs * advantages).mean()

                    total_batch_loss = base_loss + self.pg_weight * pg_loss

                scaler.scale(total_batch_loss).backward()
                torch.nn.utils.clip_grad_norm_(
                    net.parameters(), gradient_clip_val)
            ...
            avg_base_loss = total_base_loss / len(train_loader)
            avg_pg_loss = total_pg_loss / len(train_loader)
            ...
            y_true_mapped = np.array([class_mapping[label]
                                     for label in y_true])
            y_pred_mapped = np.array([class_mapping[label]
                                     for label in y_pred])
            ...
            for cls in range(5):
                ...
                conf_matrix_cls = confusion_matrix(
                    y_true_cls_binary, y_pred_cls_binary)

                if len(np.unique(y_true_cls_binary)) >= 2:
                    auc_cls = roc_auc_score(
                        y_true_cls_binary, y_probs[cls_mask, cls * 2 + 1])
                    ap_cls = average_precision_score(
                        y_true_cls_binary, y_probs[cls_mask, cls * 2 + 1])
                else:
                    auc_cls = np.nan
                    ap_cls = np.nan
            ...
            for t in study.trials:
                ...
                for epoch_result in t.user_attrs["epoch_results"]:
                    f.write(
                        ...
                        f"Total Loss: {epoch_result['total_loss']:.4f}, "
                        f"Base Loss: {epoch_result['base_loss']:.4f}, "
                        f"PG Loss: {epoch_result['pg_loss']:.4f}, "
                        f"Reward Baseline: {epoch_result['reward_baseline']:.4f}\n"
                    )
                    ...
                    for cls in range(5):
                        f.write(f"    Class {cls} \n")
                        f.write(
                            f"      Precision: {epoch_result[f'cls_{cls}_precision']:.4f}\n")
                        f.write(
                            f"      Recall: {epoch_result[f'cls_{cls}_recall']:.4f}\n")
                        f.write(
                            f"      F1: {epoch_result[f'cls_{cls}_f1']:.4f}\n")
                        f.write(
                            f"      AUC: {epoch_result[f'cls_{cls}_auc']:.4f}\n")
                        f.write(
                            f"      AP: {epoch_result[f'cls_{cls}_ap']:.4f}\n")
                        f.write("      Confusion Matrix:\n")
