...
- [ ] T1：权重/聚类/关联规则无泄漏（给出可审计说明 + 评估协议链接）
- [ ] T2：事件级切分（防止同事件跨集合）
- [ ] T4：Placebo（零分布 + p 值，p<0.01）
- [ ] T6：ERA5 海洋条件化后 FPR 显著下降（目标 <0.10）
- [ ] T5：噪声预算 + 时空聚合降噪（支撑 Table S5 的“方法学动机翻转”）
...
- [ ] Title/Abstract：从“precursor detection”收缩为“anomaly characterization/evaluation protocol”
- [ ] Introduction：问题定义从“单像元可测”转为“强混杂背景下的可检验评估”
- [ ] Results：前瞻验证（26 事件）提升为主证据叙事（不必搬动图号）
- [ ] Discussion：机制描述一律作为 *possible surface-coupling pathway*，并强调浅表敏感性限制
- [ ] Limitations：明确单传感器局限、海洋高 FPR、适用边界（M≥7.0、特定环境）

## 5. Supplementary（同一 tex 内）新增条目与编号

在 `\section*{Supplementary Figures and Tables}` 的 itemize 中追加：
- [ ] Figure S5：Placebo MCC 零分布
- [ ] Figure S6：ERA5 海洋条件化前后对比
- [ ] Table S5：Fresnel/介电灵敏度对比（重新包装为方法学动机）
- [ ] Table/ Figure S7+：按任务 01/02 实际产物补齐

并确保：
- [ ] 计数器 reset 逻辑保持正确（已存在 `\setcounter{figure}{0}` 等）
- [ ] `\label{fig:figS5}` / `\label{tab:tableS5}` 等与引用一致

## 6. 验收标准（DoD）

### 6.1 结构与可编译
- [ ] `revisions-3nd/main_clean.tex` 可独立编译（不依赖 `revisions-2nd/build/`）
- [ ] 全部 `\includegraphics{...}` 文件存在于 `revisions-3nd/`

### 6.2 叙事与风险控制
- [ ] 全文（含题注）不再出现：`near-perfect` / `operational early warning` 等禁用词
- [ ] Placebo 与 ERA5 条件化结果在正文有“可检验的主张”（带 p/CI/样本量）
- [ ] Table S5 从“不可测自证”翻转为“方法学动机 + 聚合降噪论证”

### 6.3 可复现承诺一致性
- [ ] 文稿中对“评估协议公开”的描述与 `WE-FTT/evaluation_protocol/` 的实际结构一致
