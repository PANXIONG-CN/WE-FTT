...
def main():
    ...
    # 优先使用 --data_path；若未提供，则尝试已知的多分类合并数据集路径
    data_path = args.data_path
    if not data_path:
        default_candidates = [
            "/mnt/hdd_4tb_data/ArchivedWorks/MBT/FTT/updated_code/training_dataset.parquet",
            str(Path('data/processed/training_dataset.parquet')),
        ]
    ...
    if data_path:
        ...
        train_data, val_data, test_data = dataset_creator.create_training_datasets(data_path)
    else:
        # Fallback to original default in config
        base_config = get_config('base')
        logger.info(f"No data_path provided and default candidates not found. Using configured default: {base_config.TRAINING_DATASET}")
