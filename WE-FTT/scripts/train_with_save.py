...
def main():
    ...
    trainer = ModelTrainer(config, output_dir=output_dir)
    ...
    # Save results (output_dir already created in trainer initialization)
    ...
    logger.info(f"\nSaved files:")
    logger.info(f"  - best_model.pth    : Best model checkpoint (state_dict + config)")
    logger.info(f"  - results.json      : Training history and test metrics")
    ...
    logger.info(f"  checkpoint = torch.load('{output_dir}/best_model.pth')")
