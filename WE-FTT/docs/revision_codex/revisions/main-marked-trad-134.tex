...
%DIF DEL 20250811-main-elsarticle.tex   Mon Nov 17 19:31:42 2025
%DIF ADD main_clean.tex                 Mon Nov 17 19:51:04 2025
...
% Color support for diff markup
\usepackage{xcolor}

% Journal (optional placeholder)
\journal{Remote Sensing of Environment}

% For visible strikeout in deletions
...
%DIF DIFCODE_TRADITIONAL %DIF PREAMBLE
...
% Override TRADITIONAL style to use blue additions and red strikeout deletions
\renewcommand{\DIFaddtex}[1]{\textcolor{blue}{#1}}
\renewcommand{\DIFdeltex}[1]{\textcolor{red}{\sout{#1}}}
...
Our findings demonstrate a robust pathway to resolving the signal ambiguity problem that has challenged satellite-based earthquake monitoring for decades. The key insight, supported by the near-perfect support values for specific channel combinations in distinct environmental zones (Table~\ref{tab:table1}), is that reliable pre-seismic signatures in MBT data are not universal but are fundamentally environment-specific. By abandoning the search for a single, global precursor pattern and instead embracing environmental heterogeneity, our framework successfully transforms what appears as intractable noise in global analyses into clear, context-dependent indicators of potential seismic preparation \citep{qiSpatiotemporallyWeightedTwostep2020,qiMicrowaveBrightnessTemperature2022,wuIdentifyingSeismicAnomalies2024}. This paradigm shift is validated by the performance of the WE-FTT model. The performance gain (MCC: 0.74→0.84) represents a 13.5\% relative improvement and, more importantly, corresponds to a significant enhancement in the trade-off between false positives and true positives. This demonstrates that accounting for environmental context is not merely an incremental improvement but is essential for achieving reliable detection in satellite remote sensing of geohazards.

\subsection{Physical Interpretation of MBT Anomalies and Comparison with Prior Studies}\label{sec:discuss_physical}

\DIFaddbegin \subsubsection{\DIFadd{Environment-dependent frequency--polarization responses}}
\DIFaddend The observed frequency-dependent anomaly patterns provide compelling evidence for multiple, interconnected seismic preparation mechanisms operating across different environmental contexts. \DIFaddbegin \DIFadd{Zone-dependent frequency--polarization responses (Table~\ref{tab:table1}) naturally arise from stress-activated charge carriers and dielectric perturbations in soils (wetlands and arid lands), whereas in marine settings higher-frequency channels emphasize sea-surface roughness and skin-temperature modulations, producing distinct spectral--polarization signatures. }\DIFaddend In wetland and arid zones (Zones D and E), the significant detection capabilities of low-frequency channels (e.g., 6.9\DIFaddbegin \DIFadd{~}\DIFaddend GHz H-pol) strongly support the P-hole activation hypothesis, where stress-induced positive charge carriers migrate from the crust to the surface, modifying subsurface dielectric properties \citep{freundPreearthquakeSignalsUnderlying2011,maoImpactCompressiveStress2020}. The deeper penetration of lower frequencies enables quasi-tomographic monitoring of these charge distributions, a mechanism amplified in moisture-rich soils \citep{jingChangesSurfaceWater2022}. Conversely, marine environments (Zone A) exhibit distinct signatures in high-frequency channels (e.g., 89\DIFaddbegin \DIFadd{~}\DIFaddend GHz H-pol), which is consistent with models of thermal energy transfer or microscale surface roughness changes induced by submarine fault activity or gas seepage \citep{liuGeneralFeaturesMultiparameter2023}. The systematic polarization dependence observed across all zones, highlighted in Figure~\ref{fig:fig3}, further provides a powerful discriminator between genuine precursors and meteorological noise, which typically affects both polarizations more uniformly \citep{lixinProgressesPossibleFrontiers2022}.

\DIFaddbegin \subsubsection{\DIFadd{Separating precursor signals from background variability}}
\DIFaddend Our multi-channel, environment-specific framework represents a significant methodological advance over prior remote sensing studies \DIFaddbegin \DIFadd{by explicitly quantifying how precursor signatures rise above background variability}\DIFaddend . Previous approaches often relied on single-channel analysis or fixed anomaly thresholds. For example, some studies focused on regional, threshold-based detection, while others employed wavelet analysis on individual frequency bands \citep{liuPreearthquakeMBTAnomalies2023,wuIdentifyingSeismicAnomalies2024}. While valuable, these approaches cannot capture the synergistic information we identified through association rule mining \DIFaddbegin \DIFadd{and support-difference analysis}\DIFaddend . Our results (\DIFdelbegin \DIFdel{Fig.~\ref{fig:fig2} }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig:fig2} and Table~\ref{tab:table1}}\DIFaddend ) demonstrate that certain channel combinations achieve near-perfect detection reliability *only when analyzed together*\DIFaddbegin \DIFadd{, and only after statistically bounding background variability using support-difference confidence intervals and global false-positive mapping (Section~3.6, Figure~\ref{fig:fig13})}\DIFaddend . This suggests non-linear interactions between different penetration depths and surface polarization sensitivities. \DIFdelbegin \DIFdel{This approach }\DIFdelend \DIFaddbegin \DIFadd{By combining environment-specific frequency--polarization pairs with stringent statistical controls on support-differences and false-positive rates, the framework }\DIFaddend provides a quantitative solution to the fundamental challenge of discriminating subtle local seismic changes from overwhelming ambient fluctuations, a problem articulated by \citet{maedaDiscriminationLocalFaint2008} but previously unresolved at a global scale. These environment-specific mechanisms underscore the necessity of our stratified approach, as a universal detection algorithm would inevitably miss these nuanced, zone-dependent signatures. \DIFaddbegin \DIFadd{Notably, the magnitude- and depth-stratified performance trends obtained under this framework (Section~3.6) are broadly consistent with TEC-based threshold studies across latitudinal zones \citep{shahTotalElectronContent2020}, suggesting that the MBT-based precursors we identify respect seismologically motivated scaling relationships rather than contradicting them.
}\DIFaddend 
...
\caption{Comprehensive model performance analysis \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with PCA-based visualization of metric relationships. }\DIFaddendFL PCA \DIFaddbeginFL \DIFaddFL{is used here solely for }\DIFaddendFL visualization \DIFaddbeginFL \DIFaddFL{and is not treated as a predictive baseline}\DIFaddendFL .}\label{fig:fig6}
\end{figure*}
\clearpage
...
\small
\centering
\caption{Segmented MBT ranges and mean support values across terrain zones. \DIFaddbeginFL \DIFaddFL{Note: 655.35~K denotes the AMSR-2 fill-value upper bound; all pixels with this value were removed during preprocessing and do not contribute to any of the reported statistics.}\DIFaddendFL }\label{tab:table1}
\resizebox{\textwidth}{!}{%
...
\clearpage
\section*{\DIFadd{Supplementary Figures and Tables}}

\begin{center}
{\Large \DIFadd{\textbf{Resolving signal ambiguity in earthquake precursor detection with an environment-specific deep learning framework for AMSR-2 data}}}\\[0.75em]
\DIFadd{Pan Xiong, Cheng Long, Huiyu Zhou, Roberto Battiston, Angelo De Santis, Xuhui Shen
}\end{center}
...
\begin{itemize}
    ...
    \item \DIFadd{Figure S1: Temporal evolution and performance metrics for the 26 out-of-sample M$\ge$7.0 earthquakes (2023--2025).
    }\item \DIFadd{Figure S2: Global spatial coverage of the 26 out-of-sample M$\ge$7.0 earthquakes used for forward-looking validation.
    }\item \DIFadd{Figure S3: Marine control experiment assessing the impact of tsunami-related and extreme wave conditions on MBT detections.
    }\item \DIFadd{Figure S4: Magnitude--depth--stratified performance of the WE-FTT framework.
    }\item \DIFadd{Table S1: Zone-wise statistics of false positive rate (FPR) under randomly sampled non-seismic days.
    }\item \DIFadd{Table S2: Forward-looking performance on 26 out-of-sample M$\ge$7.0 earthquakes (2023--2025).
    }\item \DIFadd{Table S3: Marine zone controls under different filtering conditions.
    }\item \DIFadd{Table S4: Stratified performance (2$\times$2) by magnitude and depth for M$\ge$7.0 earthquakes.
}\end{itemize}
}

%DIF >  Reset counters for supplementary numbering
\setcounter{figure}{0}
...
\caption{\DIFaddFL{Forest plots of MCC (left) and FPR (right) by magnitude and depth strata for M$\ge$7.0 earthquakes. Points denote estimates and horizontal whiskers denote 95\% confidence intervals. Vertical dashed lines mark the overall means (MCC = 0.804; FPR = 0.038). Strata sample sizes: shallow (0--70~km) $\times$ M7.0--7.4: $n=70$; shallow $\times$ M$\ge$7.5: $n=38$; deep ($\ge$70~km) $\times$ M7.0--7.4: $n=28$; deep $\times$ M$\ge$7.5: $n=18$. Performance mildly decreases with greater depth, consistent with TEC-based threshold studies across latitudinal zones.}}
\label{fig:figS4}
\end{figure*}
\begin{table*}[p]
\footnotesize
\centering
\caption{\DIFaddFL{Zone-wise per-pixel false-positive rate (FPR) statistics on random non-seismic days. Medians, interquartile ranges }[\DIFaddFL{Q1, Q3}]\DIFaddFL{, and 95\% confidence intervals (CI) for the mean FPR are reported for each environmental zone.}}
\label{tab:tableS1}
\begin{tabular}{@{}llccc@{}}
\toprule
\DIFaddFL{Zone }& \DIFaddFL{Name          }& \DIFaddFL{Median FPR }& [\DIFaddFL{Q1, Q3}]        & \DIFaddFL{Mean FPR (95\% CI)        }\\
\midrule
\DIFaddFL{A    }& \DIFaddFL{Marine Zone   }& \DIFaddFL{0.242      }& [\DIFaddFL{0.200, 0.275}]  & [\DIFaddFL{0.239, 0.242}] \\
\DIFaddFL{B    }& \DIFaddFL{Humid Forest  }& \DIFaddFL{0.083      }& [\DIFaddFL{0.067, 0.108}]  & [\DIFaddFL{0.085, 0.087}] \\
\DIFaddFL{C    }& \DIFaddFL{Dry Forest    }& \DIFaddFL{0.125      }& [\DIFaddFL{0.100, 0.142}]  & [\DIFaddFL{0.122, 0.124}] \\
\DIFaddFL{D    }& \DIFaddFL{Wetland       }& \DIFaddFL{0.033      }& [\DIFaddFL{0.025, 0.050}]  & [\DIFaddFL{0.036, 0.037}] \\
\DIFaddFL{E    }& \DIFaddFL{Arid Land     }& \DIFaddFL{0.100      }& [\DIFaddFL{0.075, 0.117}]  & [\DIFaddFL{0.099, 0.101}] \\
\bottomrule
\end{tabular}
\end{table*}
\clearpage

%DIF >  Table S2: per-event forward-looking performance (26 out-of-sample earthquakes)
\begin{table*}[p]
\footnotesize
\centering
\caption{\DIFaddFL{Forward-looking performance on 26 out-of-sample M$\ge$7.0 earthquakes (2023--2025). For each event, we report location, magnitude, focal depth, environmental zone, and the WE-FTT performance metrics (MCC, F1, precision, and recall) computed with the same evaluation protocol as in the main text.}}
\label{tab:tableS2}
\resizebox{\textwidth}{!}{%
\renewcommand{\arraystretch}{1.1}%
...
\footnotesize
\centering
\caption{\DIFaddFL{Marine zone controls under different filtering conditions. Detection rate, false positive rate (FPR), MCC, and $\Delta$MCC (relative to the in-sample baseline MCC = 0.84) are reported for baseline conditions, pre-earthquake-only windows, exclusion of tsunami-arrival windows, exclusion of high sea-state days, and the combination of both filters.}}
\label{tab:tableS3}
\begin{tabular}{@{}lcccc@{}}
\toprule
\DIFaddFL{Condition      }& \DIFaddFL{Detection Rate }& \DIFaddFL{FPR   }& \DIFaddFL{MCC   }& \DIFaddFL{$\Delta$MCC (vs 0.84) }\\
\midrule
\DIFaddFL{Baseline       }& \DIFaddFL{0.508          }& \DIFaddFL{0.040 }& \DIFaddFL{0.822 }& \DIFaddFL{-0.018                }\\
\DIFaddFL{Pre-EQ Only    }& \DIFaddFL{0.497          }& \DIFaddFL{0.036 }& \DIFaddFL{0.805 }& \DIFaddFL{-0.035                }\\
\DIFaddFL{No Tsunami     }& \DIFaddFL{0.495          }& \DIFaddFL{0.034 }& \DIFaddFL{0.796 }& \DIFaddFL{-0.044                }\\
\DIFaddFL{No Wind        }& \DIFaddFL{0.495          }& \DIFaddFL{0.032 }& \DIFaddFL{0.803 }& \DIFaddFL{-0.037                }\\
\DIFaddFL{Both Filtered  }& \DIFaddFL{0.486          }& \DIFaddFL{0.028 }& \DIFaddFL{0.794 }& \DIFaddFL{-0.046                }\\
\bottomrule
\end{tabular}
\end{table*}
\clearpage
...
\footnotesize
\centering
\caption{\DIFaddFL{Stratified performance (2$\times$2) by magnitude and depth for M$\ge$7.0 earthquakes. We report MCC and FPR with 95\% confidence intervals (CI). $\Delta$ values are computed relative to the overall mean (window-weighted) performance (MCC = 0.804; FPR = 0.038).}}
\label{tab:tableS4}
\resizebox{\textwidth}{!}{%
