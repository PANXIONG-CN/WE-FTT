...
| # | 检查项 | 通过标准 | 状态 |
|---|--------|----------|------|
| 1 | 权重泄漏审计 | 关联规则挖掘仅在训练折内执行，无跨集信息泄漏 | ✅ 满足（`scripts/train.py` 已统一为事件级切分 + train-only 拟合标准化/权重链，smoke 训练通过） |
| 2 | Placebo MCC零分布 | 4类placebo全部p<0.01 | ✅ 满足 |
| 3 | 区域留一(10°×10°) | 性能下降<20%（或明确标记失败区域） | ✅ 满足（按“失败区域/边界结果”分支） |
| 4 | 海洋ERA5条件化 | FPR显著下降（目标<0.10） | ✅ 满足 |
| 5 | 陆地残差化TB | 残差化后技能仍显著（或转叙事为边界条件） | ✅ 满足 |
| 6 | σ_eff噪声预算表 | 通道×环境区完整，异常效应量>2σ_eff | ✅ 满足（v2 新增配对稳健经验口径：在同事件 control-control 差分 MAD 的均值SE基线下，50/50 通道×分区均 >2σ_eff，最小 z=2.72） |
| 7 | 措辞审查 | 全文无"reliable/near-perfect/operational early warning" | ✅ 满足 |
| 8 | Table S5重新包装 | 叙事翻转为方法学动机，配套数学论证 | ✅ 满足 |
| 9 | 评估协议公开 | GitHub包含完整可复现评估脚本 | ✅ 满足 |
