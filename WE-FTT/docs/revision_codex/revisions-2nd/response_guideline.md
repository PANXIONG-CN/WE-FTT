...
- 引文：
  ...
  - 在 `\revchange` 中如果需要强调新增/修改的参考文献：
    - 推荐直接给出完整条目（作者、年份、期刊、卷页），例如：  
      `Shah, M., Athar, A. C., Tariq, M. A., et al. (2020), Possible ionosphere and atmosphere precursory analysis related to Mw > 6.0 earthquakes in Japan, Remote Sensing of Environment, 239, 111620.`  
    - 如果回复文件与主稿分开编译（例如不共享 BibTeX），**不要**在回复文件中使用 `\cite` 系列宏，以免出现 `??` 或 undefined citation；改用纯文本形式给出文献信息。

---

### 3.3 图表与大改动的推荐写法（实践增强版）

在基于图表或大段落的评论中，推荐在 `\revchange{...}` 中采用如下结构，以便审稿人只看回复信就能了解修改内容：

1. **文本类实质修改：Original + Revised**
   - 对于涉及科学含义、方法、结论等的重大修改，建议给出“原文/修改后”对照：
     ```tex
     \revchange{%
     \textbf{Change (R1-2) in the manuscript.}
     In the Introduction (Section~1, p.~4), the original sentence read:
     \begin{quote}
     \textit{Original:} ``However, traditional GPS-based Total Electron Content (TEC) monitoring is limited by the spatiotemporal migration effects of low-Earth-orbit (LEO) satellite pierce points ...''
     \end{quote}
     In the revised manuscript, it has been changed to:
     \begin{quote}
     \textit{Revised:} ``However, traditional GPS/GNSS-based Total Electron Content (TEC) monitoring relies on signals from medium-Earth-orbit (MEO) satellites, whose ionospheric pierce points (IPPs) migrate rapidly over the ground. This moving-IPP geometry introduces strong spatiotemporal aliasing, making it difficult to distinguish the temporal evolution and spatial gradients of ionospheric anomalies.''
     \end{quote}
     }
     ```
   - 对于纯 typo 或极小格式改动（例如 `dkm` → `d_i`），可只用一句话描述，不必贴出原文全文。

2. **图相关评论：完整 caption +（可选）嵌图**
   - 当审稿人针对某个图提出修改（例如 IPP 标记、滤波/小波顺序、STD 含义），在 `\revchange` 中推荐：
     - 简要说明原图存在的问题；
     - 给出**修改后的完整图注**（caption），并保证与修改稿中的图注文本一致；
     - 在回复信中嵌入更新后的 PNG 图像，并用 `\caption*` 手写图号和完整图注，避免依赖 `\label`/`\ref`，例如：
       ```tex
       \begin{figure}[H]
         \centering
         \includegraphics[width=\textwidth]{f4_fix.png}
         \caption*{\small \textbf{Figure 4.} (a) Location of the earthquake, with black triangles representing CMONOC sites and red hexagons representing the IPPs of the corresponding BeiDou-5 GEO satellite. (b) Detrended TEC time series (without band-pass filtering) from the BeiDou-5 GEO satellite for the YNWS, YNSM, and YNGM stations together with their wavelet power spectra. The 2--8 mHz band (indicated by dashed lines) is used to design the Butterworth band-pass filter applied in panel (c) and subsequent figures. (c) Time--distance plot of co-seismic ionospheric disturbances detected by the BeiDou-5 GEO satellite, based on 2--8 mHz band-pass filtered TEC.}
       \end{figure}
       ```
   - 图号建议**直接写死**，与主稿编号一致（如 Figure 4），而不要在回复信中使用 `\ref{fig:...}`，以免出现 `Figure ??`。

3. **参考文献：完整条目**
   - 对于“增加/调整参考文献”类评论，推荐在 `\revchange` 中直接给出新增文献的完整条目，例如：
     ```tex
     We have added the following references to the reference list and cited them in the Introduction:
     \begin{quote}
     Li, T., Gao, Y., Chen, C.-H., Zhang, X., and Sun, Y.-Y. (2023),
     Ionospheric Disturbances Observed over China after the 15 January 2022 Tonga Volcano Eruption,
     \textit{Geophysical Journal International}, 235, 909--919.\\[0.25em]
     Chen, C.-H., Sun, Y.-Y., Xu, R., et al. (2022),
     Resident waves in the ionosphere before the M6.1 Dali and M7.3 Qinghai earthquakes of 21--22 May 2021,
     \textit{Earth and Space Science}, 9, e2021EA002159.
     \end{quote}
     ```
   - 不依赖主稿的 BibTeX 交叉引用，保证回复信单独编译也不会出现引用错误。

4. **页码（p.~NN）的推荐做法**
   - 页码应基于**最终主稿 PDF**，例如 `p.~4`，而不是草稿版。
   - 如需减少人工工作量，可以：
     1. 使用 `pdftotext` 将主稿 PDF 转成纯文本，保留分页信息；
     2. 从 `\revchange` 中选取一段 Revised 文本作为锚点（例如某句的前 40--80 个字符），在 PDF 文本中搜索；
     3. 根据锚点所在页自动生成“comment → 页码”映射表，再回填到 `p.~NN`；  
     4. 最后人工抽查若干条，确认页码与内容对应。
   - 行号通常可选，如果期刊不强制，可仅给出页码。
...
- 对**所有实质性修改**（包括新增实验、重写段落、改图、改表）：
  - 必须在 `\revchange{...}` 中**贴出修改后的关键文本**（至少给出修订后完整段落或段落的关键部分，而不是只给一句话摘要）；
  - 必须指明**章节 + 页码（`p.~nn`）+ 位置**，例如：`In Section~3.2 (p.~15), the second paragraph now reads as follows:`；
  - 如涉及图表，还要说明**图表编号和核心内容**，并在回复中**嵌入对应图/表**，使用与主稿一致的图号。

- 对**涉及大幅重写或新增段落**的评论（例如 Introduction/Discussion 的重写、Section 结构调整等），推荐采用“Original + Revised” 对照写法：
  - `Original`：从修改前版本（例如 `main.tex` 或上一次提交的 clean 稿）中原样拷贝相关段落；
  - `Revised`：从当前 clean 稿（例如 `main_clean.tex`）中原样拷贝对应的修订后段落；
  - 如原文段落过长，可只截取与该条评论强相关的部分，但必须保证 Reviewer 可以在 PDF 中顺利定位。
...
在涉及图/表的大改动时，推荐在 `\revchange` 中直接给出图/表环境（仅用于回复信，不依赖主稿的交叉引用），图号在 caption 文本中“写死”，确保与 clean 稿一致，例如：

```tex
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/fig6_new.png}
  \caption{Figure 6. Time--distance diagrams of dTEC (TECU) along longitudinal slices at 108$^\circ$E (left) and 122$^\circ$E (right). Color indicates dTEC (TECU). The horizontal axis denotes UT, and the vertical axis denotes great-circle distance from the launch center. Vertical dashed lines indicate the launch time (13:14~UT).}
\end{figure}
```

注意：

- 回复信中的图号以 caption 文本中的 `Figure 6.` 为准，而不是依赖 `\ref` 自动编号，这样可以与 clean 稿的固定编号保持一致；
- 对 Supporting Information 中的图表，可使用 `Figure S1.`, `Table S1.` 等写法，同样在 caption 中写死编号；
- 为避免混淆，推荐在 `\revchange` 的文字部分同时给出位置说明，例如：`Figure 6 (p.~18) is reproduced below for the reviewer’s convenience.`。
